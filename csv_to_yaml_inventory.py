...
"""
Category: Python Nornir Config Script
...
def read_devices(devices_filename):
    ...
    # location list to match hostnames against for group assingment
    locations = ['xxx', 'yyy', 'zzz']
    ...
    for device_line in devices_file:
        ...
        # list comprehenssion used for checking if hostname contains string from list locations
        loccheck = [loc for loc in locations if(loc in device_info[1])]
        location = ' '
        ...
        if "sw-nx" in device_info[1]:
            ...
            # store our device in the devices dictionary
...
def filter_location():
    while True:
        sw_loc = input(
            'Location of Switches (Please enter location code!) Leave blank for all switches: ')
...
def filter_switch_type():
    while True:
        sw_type = input(
            'Type of Switches (Please enter Type code f.e. "a" for access switch): ')
        if sw_type in ('a', 'c'):
            break
        else:
            sw_type = "''"
...
def filter_switch_model():
    while True:
        sw_model = input(
            'If you want to filter to certain Model, please Type in Model f.e. 2960X: ')
...
if sw_type == "''":
    output_yml = input(
        'Please enter desired Filename for YAML-Inventory SW-Type empty recognized: ')
    command = 'tail -n +2 /opt/nms/etc/kl | grep -i {}sw- | grep -i {} | cut -d, -f1-2,4'.format(
        sw_loc, sw_model)
    ...
    command = 'tail -n +2 /opt/nms/etc/kl | grep -i {}sw- | grep -i ,{}, | grep -i {} | cut -d, -f1-2,4'.format(
        sw_loc, sw_type, sw_model)
    kl = subprocess.getoutput(command)
    devices = read_devices(kl)
